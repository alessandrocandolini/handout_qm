...
\begin{theorem}[topology of a metric space]
...
Proof is left as exercise. Proof that countable intersection don't need to be open is also left as an exercise. \Cref{theorem:topology_metric_space} defines a structure of ``topological space'' on $A$, \ie, a ordered pair of $(A, \tau)$ where $\tau$ is a set of subsets of $A$ closed under arbitrary unions and finite intersections. 
...
different from $\varphi$ itself. 
  ...
  \begin{definition}[limit]
    ...
    ``limit of $f$ at $\varphi$'', and it's written $\lim_{\psi \rightarrow \varphi} f$, if 
   \begin{dmath}[label={limit}]
      (\forall \varepsilon > 0)
      (\exists \delta > 0 )
      (\forall \psi \in D) 
      (\psi \neq \varphi) 
  ...
  \Cref{def:continuity,def:limit} might not look particularly useful in practice. In the case of \cref{def:limit}, while \eqref{eq:limit} determines unambiguously whether a candidate $l$ is or is not the limit, the identification of a target $l$ itself needs to happen by some other mean. It's possible to develop a calculus of limits and continuous functions by \begin{inparaenum}[a)] 
  \item develop an ``alphabet'' of known limits for the simplest possible cases of practical relevance, relying solely on \eqref{def:limit};
