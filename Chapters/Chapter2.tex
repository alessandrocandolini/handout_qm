...
Metric spaces provide a natural context where to develop concepts like continuity, convergence, and more.
  ...
  \begin{definition}[neighbourhood]
    ...
    subset  $A' \subset A$ for which there exists a open ball entirely contained in $A'$, \ie, 
  ...
  Given a metric space $(A,d)$, every subset of $A$ is a metric space with respect to the ``restriction'' of $d$ to $A'$. It's thus meaningful to talk about ``open subsets'' of $A$. 
  ...
  \end{proposition}
\begin{proof}
  Let $(A,d)$ be a metric space, and consider the open ball $B_{\rho}(\varphi)$. The goal is to prove that for every $\psi \in B_{\rho}(\varphi)$, there exists $\rho' > 0$ such that $B_{\rho'}(\psi) \subseteq B_{\rho} (\varphi)$. Let $\rho' = \rho - d(\varphi, \psi)$, then for every $\xi \in B_{\rho'}(\psi)$ using the triangle disequality we have $d(\xi, \varphi) \leq d(\xi , \psi) + d(\psi, \varphi) < \rho - d(\varphi, \psi) + d(\psi, \varphi) = \rho$. 
\end{proof}
