...
particular, the notion of orthogonality) which are not present in general
Banach spaces. 
...
\begin{theorem}[Jordan-von Neumann theorem]
   ...
   $(X, \norm{\cdot})$ be a Banach space over $\K$. %normed vector space over $\K$. 
   The norm $\norm{\cdot}$ comes from an inner product (\ie, the Banach space
   is also an Hilbert space) \emph{if and only if}
...
\begin{exercise}
   Does \cref{thm:parallelogram_law} apply to generic normed vector spaces over
   $\K$ which are not completed (\ie, which are not Banach spaces)?
\end{exercise}
...
\end{dmath}.
It is possible to prove that this function indeed is a inner product on
$\C^{N}$.
We need to verify
\cref{item:inner1,item:inner2,item:inner3,item:inner4,item:inner5} of
\cref{def:inner_product}.
Linearity is traightforward:
\begin{dgroup*}
   \begin{dmath*}[compact]
      \braket{\vec{\varphi}|\vec{\psi} + \vec{\eta}} = 
      \sum_{k=1}^{N} \conj{\varphi_{k}} \left( \psi_{k} + \eta_{k}\right) = 
      \left( \sum_{k=1}^{N} \conj{\varphi_{k}} \psi_{k} \right) + 
      \left( \sum_{k=1}^{N} \conj{\varphi_{k}} \eta_{k}\right)
      = \braket{\vec{\varphi}|\vec{\psi}} + \braket{\vec{\varphi} | \vec{\eta}}
      \condition{$\forall ( \vec{\varphi}, \vec{\psi}, \vec{\eta}) \in
	 C^{N}\times C^{N} \times \C^{N}$  }
   \end{dmath*},
   \begin{dmath*}[compact]
      \braket{\vec{\varphi}|\lambda \vec{\psi} + \vec{\eta}} = 
      \sum_{k=1}^{N} \conj{\varphi_{k}} \lambda \psi_{k} = 
      \lambda 
      \sum_{k=1}^{N} \conj{\varphi_{k}} \psi_{k} = 
      \lambda \braket{\vec{\varphi}|\vec{\psi}} 
      \condition{$\forall ( \vec{\varphi}, \vec{\psi}) \in
	 C^{N}\times C^{N} $ and $\lambda \in \C$  }
   \end{dmath*}.
\end{dgroup*}
\Cref{item:inner3} follows from the fact that, for every $(z,w) \in \C \times
\C$, 
\begin{itemize}
   \item $\conj{\left( z+w\right)}  = \conj{z} + \conj{w}$;
   \item $\conj{
   \left( zw\right)}= \conj{z} \conj{w}$; 
	\item $\conj{ ( \conj{z})} = z$.
\end{itemize}
(It is easy to prove by
induction on $N$ that the first property is true also with a sum of $N$ complex
numbers.)
Thus:
\begin{dmath*}
   \braket{\vec{\psi}|\vec{\varphi}} = 
   \sum_{k=1}^{N} \conj{\psi_{k}} \varphi_{k} = 
\sum_{k=1}^{N} \conj{\left( \psi_{k} \conj{\varphi_{k}}  \right)}
	    = 
	    \conj{ \left( 
		  \sum_{k=1}^{N} \conj{\varphi_{k} } \psi_{k}
	       \right)}
