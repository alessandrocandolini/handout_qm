...
Metric spaces provide a natural context where to develop concepts like continuity, convergence, and more.
  ...
  \begin{exercise}[discrete metric space]
  ...
  The next sections will provide examples of metric spaces where the distance arises from additional structure (\ie, norm, inner product). Without pretending to extensively develop the theory of metric spaces in this section, it's worth briefly mentioning that notions such as continuity, convergence, and more can be formulated quite generally in the context of metric spaces.
  ...
  \begin{definition}[continuity]
    ...
    \end{dmath}.
  \end{definition}
  The notion of continuity can be further generalised to functions between \emph{topological spaces}, where there is generally no formal notion of distance.
  Equivalent characterisations of continuity in metric spaces will be presented shortly, after discussing the ``topology'' of metric spaces. 

  First of all, few propedeutic definitions to build terminology: open balls, neighbourhoods, isolated points, dense subsets, open and closed metric spaces. 

  \begin{definition}[open ball]
    Let $(A,d)$ be a metric space.
    Given $\rho > 0$ and $\varphi\in A$, 
    the ``open ball'' $B_{\rho}(\varphi)$ of ``center'' $\varphi$ and ``radius'' $\rho$ is the subset of $A$ defined by 
    \begin{dmath}
      B_{\rho}(\varphi) = \Set { \psi \in A | d (\varphi, \psi) < \rho } 
    \end{dmath},
    \ie, it's the subset of \emph{all and only} the points of $A$ at a distance less than $\rho$ from $\varphi$.
  \end{definition} 

  \begin{definition}[neighbourhood]
    Let $(A,d)$ be a metric space. 
    A ``neighbourhood'' of a point $\varphi\in A$ is any 
    non empty subset  $A' \subset A$ with the property that 
    there exist $\rho > 0 $ such that $B_{\rho}(\varphi) \subseteq A'$.
  \end{definition}
  \begin{definition}[isolated point]
    Let $(A,d)$ be a metric space. 
    A point  $\varphi\in A$ is ``isolated'' if there exists $\rho > 0$ such that $B_{\rho}(\varphi) \cap A \backslash \{ \varphi \} = \varnothing$ (\ie, there are no other points of $A$ closer to $\varphi$ for less than $\rho$).
  \end{definition}

  \begin{definition}[open metric space] 
    A metric space $(A,d)$ is ``open'' if $A$ is a neighbourhood of every point of $A$, \ie, if for every $\varphi \in A$ there exists some $\rho > 0 $ such that $B_{\rho}(\varphi) \subseteq A$. 
  \end{definition}

  Given a metric space $(A,d)$, every $A' \subseteq A$ can be turned into a metric space $(A', d')$ where $\fullfunction{d'}{A' \times A'}{\R}$ is the ``restriction'' of $d$ to $A'$, \ie, $d' (\varphi,  \psi) = d (\varphi, \psi)$ for all $(\varphi, \psi) \in A' \times A'$. Because of this, it's meaningful to talk about ``open subsets'' of $A$. 
